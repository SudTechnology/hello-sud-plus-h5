...
import { getCode } from 'api/dataGame' // 短期令牌code接口
...
/**
 * 数值游戏(CP)专用的 FSM 状态字面量。
 * sudgip-sdk-js-wrapper-test@0.1.10 的 SudGIPMGState / SudGIPAPPState 里没有这些常量，
 * 取值与安卓工程 tech.sud.mgp.cp 的 SudMGPMGState / SudMGPAPPState 逐字一致。
 */
/** 游戏 -> APP */
export const MG_STATE = {
  /** 游戏通知 APP 获取积分 */
  GAME_GET_SCORE: 'mg_common_game_get_score',
  /** 游戏通知 APP 带入积分 */
  GAME_SET_SCORE: 'mg_common_game_set_score',
  /** 游戏通知 APP 创建订单（打赏、购买道具等） */
  GAME_CREATE_ORDER: 'mg_common_game_create_order'
} as const

/** APP -> 游戏 */
export const APP_STATE = {
  /** 回传积分余额，data: { score: number } */
  GAME_SCORE: 'app_common_game_score',
  /** 回传下单结果，data: { result: 1 成功 | 0 失败 } */
  GAME_CREATE_ORDER_RESULT: 'app_common_game_create_order_result'
} as const
...
export class SDKGameView {
  ...
  public login(userId: string) {
    return new Promise<void>((resolve, reject) => {
      ...
      getCode(data).then(async (res) => {
        ...
        // /login/v1 返回 ret_code:0，服务端账户已创建，等价于安卓 CustomGameActivity 的 onLoginSuccess。
        // 必须等这里 resolve 之后再调 /get-account/v1，否则服务端返回 1013 RET_ACCOUNT_NOT_EXIST。
        resolve()
      }).catch(reject)
    })
  }
  ...
  public loadGame({ userId, code }: ILoadGameParam) {
    ...
    this.sudFSMMGDecorator.setSudFSMMGListener({
      ...
      onGameStateChange(handle, state, dataJson) {
        ...
        // mg_common_game_get_score / set_score / create_order 的业务处理在 hooks/useDataGameDetail.ts 的 onGameStateChange 里（经上面这行转发）
        return false
      },
      // 监听玩家状态改变
  ...
  /**
   * 把积分余额回传给游戏。
   * 对应安卓 CustomGameActivity.notifyAppCommonGameScore(1823-1838)。
   * 注意：安卓只在响应 mg_common_game_get_score 时回传，APP 自己刷新余额时不回传。
   * iSudFSTAPP 未就绪时 decorator 内部会静默 no-op，无需额外判空。
   */
  public notifyAppCommonGameScore(coin: number) {
    this.sudFSTAPPDecorator.notifyStateChange(APP_STATE.GAME_SCORE, JSON.stringify({ score: coin }), {
      onSuccess() { },
      onFailure() { }
    })
  }
  ...
  /**
   * 把下单结果回传给游戏。游戏在等这个结果，失败也必须回传，否则会卡住游戏内流程。
   * 对应安卓 CustomGameActivity.notifyAppCommonCreateOrderResult(1917-1932)。
   * @param result 1 成功，0 失败
   */
  public notifyAppCommonCreateOrderResult(result: 0 | 1) {
    this.sudFSTAPPDecorator.notifyStateChange(APP_STATE.GAME_CREATE_ORDER_RESULT, JSON.stringify({ result }), {
      onSuccess() { },
      onFailure() { }
    })
  }
  ...
  // 根据域名生成bundleId
  public getBundleId() {
    return 'fat-hello-sud.sud.tech'// location.hostname
  }
}
