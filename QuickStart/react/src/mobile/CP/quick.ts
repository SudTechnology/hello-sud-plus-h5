...
import { SudMGP, ISudAPPD } from 'sudmgp-sdk-js-cp-test'
import { ISudMGP, ISudFSTAPP } from 'sudmgp-sdk-js-cp-test/type' // SudMGP类型
...
export class SDKGameView {
  ...
  // 初始化数据
  constructor({ root, roomId, appId, appKey, gameUrl, language = 'zh-CN', gameId, userId }: IBaseGameViewModelConstru) {
    console.log('[ url 参数 ] >', Array.from(arguments)[0])
    this.gameRoomId = roomId
    this.gameId = gameId
    this.userId = userId
    this.root = root
    language && (this.language = language)
    this.SudMGP_APP_ID = appId
    this.SudMGP_APP_KEY = appKey
